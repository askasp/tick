...
| a key on the board | a line in `lib/board.sh`: `ACTIONS` for what it does to a task, else `BOARD_KEYS`, `FORM_KEYS` or `INBOX_KEYS` (`t inbox`'s). `t ui` binds it from there, and screens name it with `key NAME` | no |
